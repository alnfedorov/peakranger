...
BgzfData::BgzfData(void)
        : UncompressedBlockSize(DEFAULT_BLOCK_SIZE), CompressedBlockSize(MAX_BLOCK_SIZE), BlockLength(0),
          BlockOffset(0), BlockAddress(0), IsOpen(false), IsWriteOnly(false), Stream(NULL), UncompressedBlock(NULL),
...
int BgzfData::DeflateBlock(void) {
    ...
    while (true) {
        ...
        if (deflateInit2(&zs, Z_DEFAULT_COMPRESSION, Z_DEFLATED, GZIP_WINDOW_BITS, Z_DEFAULT_MEM_LEVEL,
