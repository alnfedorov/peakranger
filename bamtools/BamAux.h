...
// Explicit variable sizes
    ...
    public:
        ...
        bool IsProperPair(
                void) const;        // Returns true if alignment is part of read that satisfied paired-end resolution
    ...
    public:
        ...
        void SetIsProperPair(
                bool ok);        // Sets "alignment is part of read that satisfied paired-end resolution" flag
    ...
    public:
        ...
        template<typename T>
        bool GetTag(const std::string &tag, T &destination);
    ...
    public:
        ...
        struct BamAlignmentSupportData {
            ...
            BamAlignmentSupportData(void)
                    : BlockLength(0), NumCigarOperations(0), QueryNameLength(0), QuerySequenceLength(0),
                      IsParsed(false) {}
        };
...
// Auxiliary data structs & typedefs
    ...
    struct RefData {
        ...
        RefData(const int32_t &length = 0,
                bool ok = false)
                : RefLength(length), RefHasAlignments(ok) {}
    };
    ...
    struct BamRegion {
        ...
        BamRegion(const int &leftID = -1,
                ...
                : LeftRefID(leftID), LeftPosition(leftPos), RightRefID(rightID), RightPosition(rightPos) {}
    };
...
// Manipulate alignment flags 
    inline void
    BamAlignment::SetIsDuplicate(bool ok) { if (ok) AlignmentFlag |= bDUPLICATE; else AlignmentFlag &= ~bDUPLICATE; }

    inline void
    BamAlignment::SetIsFailedQC(bool ok) { if (ok) AlignmentFlag |= QC_FAILED; else AlignmentFlag &= ~QC_FAILED; }

    inline void
    BamAlignment::SetIsFirstMate(bool ok) { if (ok) AlignmentFlag |= READ_1; else AlignmentFlag &= ~READ_1; }

    inline void BamAlignment::SetIsMateUnmapped(
            bool ok) { if (ok) AlignmentFlag |= MATE_UNMAPPED; else AlignmentFlag &= ~MATE_UNMAPPED; }

    inline void BamAlignment::SetIsMateReverseStrand(
            bool ok) { if (ok) AlignmentFlag |= MATE_REVERSE; else AlignmentFlag &= ~MATE_REVERSE; }

    inline void BamAlignment::SetIsPaired(bool ok) { if (ok) AlignmentFlag |= PAIRED; else AlignmentFlag &= ~PAIRED; }

    inline void
    BamAlignment::SetIsProperPair(bool ok) { if (ok) AlignmentFlag |= PROPER_PAIR; else AlignmentFlag &= ~PROPER_PAIR; }

    inline void
    BamAlignment::SetIsReverseStrand(bool ok) { if (ok) AlignmentFlag |= REVERSE; else AlignmentFlag &= ~REVERSE; }

    inline void BamAlignment::SetIsSecondaryAlignment(
            bool ok) { if (ok) AlignmentFlag |= SECONDARY; else AlignmentFlag &= ~SECONDARY; }

    inline void
    BamAlignment::SetIsSecondMate(bool ok) { if (ok) AlignmentFlag |= READ_2; else AlignmentFlag &= ~READ_2; }

    inline void
    BamAlignment::SetIsUnmapped(bool ok) { if (ok) AlignmentFlag |= UNMAPPED; else AlignmentFlag &= ~UNMAPPED; }
