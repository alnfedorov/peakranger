...
struct BamReader::BamReaderPrivate {
    ...
    // -------------------------------
    enum RegionState {
        BEFORE_REGION = 0, WITHIN_REGION, AFTER_REGION
    };
    ...
    void
    InsertBinEntry(BamBinMap &binMap, const uint32_t &saveBin, const uint64_t &saveOffset, const uint64_t &lastOffset);
...
BamReader::BamReaderPrivate::BamReaderPrivate(void)
        : IsIndexLoaded(false), AlignmentsBeginOffset(0), IsLeftBoundSpecified(false), IsRightBoundSpecified(false),
          IsRegionSpecified(false), CurrentRefID(0), CurrentLeft(0), DNA_LOOKUP("=ACMGRSVTWYHKDBN"),
...
bool BamReader::BamReaderPrivate::BuildCharData(BamAlignment &bAlignment) {
    ...
    const unsigned int seqDataOffset =
            bAlignment.SupportData.QueryNameLength + (bAlignment.SupportData.NumCigarOperations * 4);
    ...
    for (; cigarIter != cigarEnd; ++cigarIter) {
        ...
        switch (op.Type) {
            ...
            case ('M') :
            case ('I') :
                bAlignment.AlignedBases.append(
                        bAlignment.QueryBases.substr(k, op.Length)); // for 'M', 'I' - write bases
                // fall through
            ...
            case ('N') :
                bAlignment.AlignedBases.append(op.Length,
                                               'N');  // for 'N' - write N's, skip bases in original query sequence
                // k+=op.Length; 
                break;
...
bool BamReader::BamReaderPrivate::BuildIndex(void) {
    ...
    while (GetNextAlignment(bAlignment)) {
        ...
        if (lastRefID != bAlignment.RefID) {
            ...
            printf("Alignment %s : %d > %d on reference (id = %d)", bAlignment.Name.c_str(), lastCoordinate,
                   bAlignment.Position, bAlignment.RefID);
            exit(1);
        }
...
int64_t BamReader::BamReaderPrivate::GetOffset(std::vector<int64_t> &chunkStarts) {
    ...
    uint64_t minOffset = ((unsigned int) (Region.LeftPosition >> BAM_LIDX_SHIFT) >= offsets.size()) ? 0 : offsets.at(
            Region.LeftPosition >> BAM_LIDX_SHIFT);
    ...
    if (References.at(refID).RefHasAlignments && (position <= References.at(refID).RefLength)) {
        ...
        if (offset == -1) {
            ...
            for (std::vector<int64_t>::const_iterator o = chunkStarts.begin(); o != chunkStarts.end(); ++o) {
                ...
                if ((bAlignment.RefID == refID && bAlignment.Position + bAlignment.Length > position) ||
                    bAlignment.RefID > refID) {
