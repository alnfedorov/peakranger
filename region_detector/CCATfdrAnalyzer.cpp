...
namespace ccat_aux {
    ...
    double CCATfdrAnalyzer::ComputeLocalFDR(vector<peak_t> &l1Peaks, vector<peak_t> &l2Peaks, vector<double> &q,
                                            vector<double> &value, const ccat_config_t &config, size_t tagCount,
                                            size_t binCount) {
    ...
    int
    CCATfdrAnalyzer::PostProcessing(vector<chr_t> &chroms, int chromNum, double l1Ratio, double l2Ratio, int maxL1Count,
    ...
    int CCATfdrAnalyzer::ProcessOneChrom(chr_t &chrom, double l1Ratio, double l2Ratio, const ccat_config_t &config) {
        ...
        GenRegionProfile(profile1, profile2, region, l1Ratio, l2Ratio, config.slidingWinSize, config.movingStep,
                         config.bootstrapPass, config.minScore, row, column, tagCount, binCount, smoothingFactor,
                         lookUpTable, flag);
        ...
        for (i = 0; i < chrom.l1Peaks.size(); i++) {
            ...
            tmpIndex =
                    lower_bound(value.begin(), value.end(), chrom.l1Peaks[i].foldChange - 0.00000001, greater<double>())
                    - value.begin();
            if (tmpIndex != 0) {
