...
cppoisson::cppoisson(Matrix<double> obs1, double p1,
    ...
    beta = beta1;
    invbeta = 1.0 / beta;
    log00 = LogGamma(alpha) - alpha * log(invbeta);
    obs.newsize(N + 1, 4);
    obs = obs1;
    fFilter.newsize(N + 1, K + 1);
    bFilter.newsize(N + 1, K + 1);
    falpha.newsize(N + 1, K + 1);
    balpha.newsize(N + 1, K + 1);
    fIbeta.newsize(N + 1, K + 1);
    bIbeta.newsize(N + 1, K + 1);
    fQ.newsize(N + 1, K + 1);
    bQ.newsize(N + 1, K + 1);
    fLog.newsize(N + 1, K + 1);
    bLog.newsize(N + 1, K + 1);
    estPara.newsize(N + 1);
    BcmixForward();
    BcmixBackward();
}
...
void cppoisson::BcmixSmooth() {
    ...
    for (int t = 1; t < N; t++) {
        ...
        for (int i = 1; i <= min(t, K); i++) {
            tmpA[i] = log(p) + log(fQ[t][i]);
            if (tmpmax < tmpA[i]) tmpmax = tmpA[i];
        }
        for (int i = 1; i <= min(t, K); i++)
            for (int j = 1; j <= min(N - t, K); j++) {
...
void cppoisson::BcmixBackward() {
    ...
    for (int t = N - 1; t >= N - K + 1; t--) {
        ...
        for (int i = 1; i <= s; i++) {
            balpha[t][i] = tmpPost[i][1];
            bIbeta[t][i] = tmpPost[i][2];
            bLog[t][i] = tmpPost[i][3];
            bQ[t][i] = bQstar[i] / sumvec;
            bFilter[t][i] = N + 1 - i;
        }
    }
    for (int t = N - K; t >= 1; t--) {
        ...
        numTmp[K + 1] = tmpPost[K + 1][3] - log00;
        tmpBackFil[K + 1] = t;
        for (int i = 1; i < K + 1; i++) {
            SubFun1(tmpPost[i][1], tmpPost[i][2], tmpPost[i][3],
                    balpha[t + 1][i], bIbeta[t + 1][i], obs[t][2], obs[t][3]);
        ...
        for (int i = 1; i < minind; i++) {
            ...
            bIbeta[t][i] = tmpPost[i][2];
            bLog[t][i] = tmpPost[i][3];
            bFilter[t][i] = tmpBackFil[i];
            bQ[t][i] = bQstar[i] * delta;
        }
        for (int i = minind + 1; i <= K + 1; i++) {
            balpha[t][i - 1] = tmpPost[i][1];
            bIbeta[t][i - 1] = tmpPost[i][2];
            bLog[t][i - 1] = tmpPost[i][3];
            bFilter[t][i - 1] = tmpBackFil[i];
...
void cppoisson::BcmixForward() {
    ...
    for (int t = 2; t <= K; t++) {
        ...
        for (int i = 1; i <= t; i++) {
            falpha[t][i] = tmpPost[i][1];
            fIbeta[t][i] = tmpPost[i][2];
            fLog[t][i] = tmpPost[i][3];
            fQ[t][i] = fQstar[i] / sumvec;
            fFilter[t][i] = i;
        }
    }
    ...
    for (int t = K + 1; t < N + 1; t++) {
        ...
        for (int i = 1; i < minind; i++) {
            ...
            fIbeta[t][i] = tmpPost[i][2];
            fLog[t][i] = tmpPost[i][3];
            fFilter[t][i] = tmpForFil[i];
            fQ[t][i] = fQstar[i] * delta;
        }
        for (int i = minind + 1; i <= K + 1; i++) {
            falpha[t][i - 1] = tmpPost[i][1];
            fIbeta[t][i - 1] = tmpPost[i][2];
            fLog[t][i - 1] = tmpPost[i][3];
            fFilter[t][i - 1] = tmpForFil[i];
...
double cppoisson::LogGamma(double x) const {
    ...
    double b[98] = {-4.587497028e-11, 1.902363396e-10, 8.6377323367e-10, 1.15513678861e-8, -2.556403058605e-8,
                    -1.5236723372486e-7,
                    ...
                    -5.184290387e-11, -8.3355121068e-10, -2.56167239813e-9, 1.455875381397e-8, 1.3512178394703e-7,
                    2.9898826810905e-7,
                    ...
                    3.0279161576e-10, 1.60742167357e-9, -4.05596009522e-9, -5.089259920266e-8, -2.029496209743e-8,
                    1.35130272477793e-6,
                    ...
                    -5.091914958e-10, -1.15274986907e-9, 1.237873512188e-8, 2.937383549209e-8, -3.0621450667958e-7,
                    -7.7409414949954e-7,
                    ...
                    4.0119897187e-10, -1.3224526679e-10, -1.002723190355e-8, 2.569249716518e-8, 2.0336011868466e-7,
                    -1.1809768272606e-6,
                    ...
                    -1.5413428348e-10, 6.4905779353e-10, 1.60702811151e-9, -2.655645793815e-8, 7.619544277956e-8,
                    4.7604380765353e-7,
                    ...
                    1.049740243e-11, -2.5832017855e-10, 1.39591845075e-9, -2.1177278325e-10, -5.082950464905e-8,
                    3.7801785193343e-7,
    ...
    double c[65] = {
            1.16333640008e-8, -8.33156123568e-8, 3.832869977018e-7, -1.5814047847688e-6, 6.50106723241e-6,
            -2.74514060128677e-5,
            1.209015360925566e-4, -5.666333178228163e-4, 0.0029294103665559733, -0.0180340086069185819,
            0.1651788780501166204,
            1.1031566406452431944, 1.2009736023470742248, 1.3842760642e-9, -6.9417501176e-9, 3.42976459827e-8,
            -1.785317236779e-7,
            9.525947257118e-7, -5.2483007560905e-6, 3.02364659535708e-5, -1.858396115473822e-4,
            0.0012634378559425382, -0.0102594702201954322, 0.1243625515195050218, 1.3888709263595291174,
            2.4537365708424422209,
            1.298977078e-10, -8.02957489e-10, 4.945484615e-9, -3.17563534834e-8, 2.092136698089e-7, -1.4252023958462e-6,
            1.01652510114008e-5, -7.74550502862323e-5, 6.537746948291078e-4, -0.006601491253552183,
            0.0996711934948138193,
            1.6110931485817511402, 3.9578139676187162939, 1.83995642e-11, -1.353537034e-10, 9.984676809e-10,
            -7.6346363974e-9,
            5.99311464148e-8, -4.868554120177e-7, 4.1441957716669e-6, -3.77160856623282e-5, 3.805693126824884e-4,
            -0.0045979851178130194,
            0.0831422678749791178, 1.7929113303999329439, 5.6625620598571415285, 3.4858778e-12, -2.97587783e-11,
            2.557677575e-10, -2.2705728282e-9, 2.0702499245e-8, -1.954426390917e-7, 1.9343161886722e-6,
            -2.0479024910257e-5,
            2.405181940241215e-4, -0.0033842087561074799, 0.0713079483483518997, 1.9467574842460867884,
